...
public class CharacterController : MonoBehaviour {
	...
	public void Move(Vector2 direction) {
		if (rb.velocity.x != 0) {
			anim.SetFloat ("LastMoveX", Util.Crush (rb.velocity.x));
			anim.SetFloat ("LastMoveY", 0);
		} else if (rb.velocity.y != 0) {
			anim.SetFloat ("LastMoveY", Util.Crush (rb.velocity.y));
			anim.SetFloat ("LastMoveX", 0);
		}

		if (Mathf.Abs (rb.velocity.x) >= Mathf.Abs (rb.velocity.y)) {
			anim.SetFloat ("MoveX", Util.Crush (rb.velocity.x));
			anim.SetFloat ("MoveY", 0);
		} else {
			anim.SetFloat ("MoveX", 0);
			anim.SetFloat ("MoveY", Util.Crush (rb.velocity.y));
		}
